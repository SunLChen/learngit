...
string int_tString(int & a){
	stringstream is;
	is<<a;
	return is.str();
}
#define DEBUGARG ((string)__FILE__+"("+to_string(__LINE__)+")  "+__func__+"()  ")
 #define logwarn(msg,...) MyLog::getInstance()->warn((DEBUGARG.append(msg).append(to_string(__VA_ARGS__))).c_str())
...
int main(){
	int num=10;
	logwarn("this is a warn msg,num=",num);
